...
export class Decimal {
    ...
    static formats: {
        toSubscript: (value: number
            ...
            ,
            type?: string) => string; format: (ex: DecimalSource, acc?: number, max?: number, type?: string) => string; formatGain: (amt: DecimalSource, gain: DecimalSource) => string; formatTime: (ex: DecimalSource, acc?: number, type?: string) => string; formatReduction: (ex: DecimalSource) => string; formatPercent: (ex: DecimalSource) => string; formatMult: (ex: DecimalSource, acc?: number) => string; expMult: (a: DecimalSource, b: DecimalSource, base?: number) => Decimal; metric: (num: DecimalSource, type: number) => string; ev: (num: DecimalSource, c2?: boolean) => string; omega: { config: { greek: string; infinity: string; }; format(value: Decimal): string; }; omega_short: {
                config: { greek: string; infinity: string; }; format(value // The smallest exponent that can appear in a Number, though not all mantissas are valid here.
                    : Decimal): string;
            }; elemental: {
                config: { element_lists: string[][]; }; getOffset(group: number): number; getAbbreviation // For default toString behaviour, when to swap from eee... to (e^n) syntax.
                    (// For default toString behaviour, when to swap from eee... to (e^n) syntax.
                        group: number, progress: number): string; beyondOg(x: number): string; abbreviationLength(group: number): number; getAbbreviationAndValue(x: Decimal): (string | Decimal)[]; formatElementalPart(abbreviation: string, n: Decimal): string; format(value: Decimal, acc: number): string;
            }; old_sc: {
                ...
                format(ex: DecimalSource, acc // when exponent's absolute value is large is slightly inaccurate.
                    :
                    // You can fix it with the power of math... or just make a lookup table.
                    // Faster AND simpler
                    number): string;
                ...
                layers: string[]; format(ex: DecimalSource // Faster AND simpler
                    , // Faster AND simpler
                    acc: number, max: number): string;
            }; standard: { tier1(x: number): string; tier2(x: number): string; }; inf: { format(ex: DecimalSource, acc: number, max: number): string; };
    };
   ...
   */
    ...
    */
     ...
     */
    public mod (other: DecimalSource): Decimal {
    ...
    */
     ...
     * @returns {string} A string representing the formatted E value.
     */
    public format (acc: number = 2, max: number = 9): string { return format(this.clone(), acc, max); }
...
function formatST (ex: DecimalSource, acc: number = 2, max: number = 9, type: string = "st"): string {return format(ex, acc, max, type);}
function format (ex: DecimalSource, acc: number = 2, max: number = 9, type: string = "mixed_sc"): string {
    ex = new Decimal(ex);
    const neg = ex.lt(0) ? "-" : "";
...
function formatReduction (ex: DecimalSource): string { ex = new Decimal(ex); return format(new Decimal(1).sub(ex).mul(100)) + "%"; }

function formatPercent (ex: DecimalSource): string { ex = new Decimal(ex); return format(ex.mul(100)) + "%"; }
// @ts-ignore
function formatMult (ex: DecimalSource, acc: number = 2): string { ex = new Decimal(ex); return ex.gte(1) ? "×" + ex.format(acc) : "/" + ex.pow(-1).format(acc);}

function expMult (a: DecimalSource, b: DecimalSource, base: number = 10) { return Decimal.gte(a, 10) ? Decimal.pow(base, Decimal.log(a, base).pow(b)) : new Decimal(a); }
...
function ev (num: DecimalSource, c2: boolean = false) {
