...
import type { DataManagerEntry } from "../game/managers/DataEntry";
...
type RecordToDataEntry<TSettings> = {
    [K in keyof TSettings]: DataManagerEntry<TSettings[K]>;
};

/**
 * Creates a record of {@link DataManagerEntry} instances for each property in the provided settings object.
 ...
 * @returns A record where each key corresponds to a property in the settings object, and each value is a {@link DataManagerEntry} for that property.
 * @example
 * const settings = {
 *     ...defaultBaseSettings,
