...
import { Expose, Exclude, Type, instanceToPlain, plainToInstance } from 'class-transformer';
...
class DataManager {
    ...
    init() {
        this.normalData = this.data, this.normalDataPlain = instanceToPlain(this.data);
    }
    compileDataRaw(data = this.data) {
        this.gameRef.eventManager.dispatch("beforeCompileData");
        let gameDataString = instanceToPlain(data);
        return [
            {
        ...
        return function plainToInstanceRecursive(normal, plain) {
            ...
            for(let key in normal){
                ...
                plain[key] = function(templateClassToConvert, plain) {
                    let out = plainToInstance(templateClassToConvert, plain);
