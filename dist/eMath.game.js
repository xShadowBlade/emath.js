(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('reflect-metadata'), require('class-transformer')) :
    typeof define === 'function' && define.amd ? define(['exports', 'reflect-metadata', 'class-transformer'], factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.eMathGame = {}, null, global.classTransformer));
})(this, (function (exports, reflectMetadata, classTransformer) { 'use strict';
    ...
    let { formats} = decimalFormatGenerator(Decimal);
    ...
    class ConfigManager {
        parse(config) {
            return void 0 === config ? this.configOptionTemplate : function parseObject(obj, template) {
                for(let key in template)void 0 === obj[key] ? obj[key] = template[key] : "object" != typeof obj[key] || "object" != typeof template[key] || Array.isArray(obj[key]) || Array.isArray(template[key]) || (obj[key] = parseObject(obj[key], template[key]));
                return obj;
            }(config, this.configOptionTemplate);
        }
        get options() {
    ...
    let keys = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890 ".split("").concat([
        "ArrowUp",
        "ArrowDown",
        "ArrowLeft",
        "ArrowRight"
    ...
    class KeyManager {
        changeFps(fps) {
            this.config.fps = fps, this.tickerInterval && (clearInterval(this.tickerInterval), this.tickerInterval = setInterval(()=>{
                for (let ticker of this.tickers)ticker(1000 / fps);
            }, 1000 / fps));
        }
        logKey(event, type) {
            let key = event.key;
            type && !this.keysPressed.includes(key) ? this.keysPressed.push(key) : !type && this.keysPressed.includes(key) && this.keysPressed.splice(this.keysPressed.indexOf(key), 1);
        }
        onAll(eventType, keypress) {
            for (let bind of this.binds)if (bind.key === keypress) switch(eventType){
                case "down":
                    bind.onDown?.();
                    break;
                case "press":
        ...
        isPressing(id) {
            for (let current of this.binds)if (current.id === id) return this.keysPressed.includes(current.key);
            return !1;
        }
        getBind(id) {
        ...
        addKey(nameOrKeysToAdd, key, fn) {
            for (let keyBinding of nameOrKeysToAdd = Array.isArray(nameOrKeysToAdd = "string" == typeof nameOrKeysToAdd ? {
                id: nameOrKeysToAdd,
                name: nameOrKeysToAdd,
                key: key ?? "",
                fn
            } : nameOrKeysToAdd) ? nameOrKeysToAdd : [
                nameOrKeysToAdd
            ]){
                keyBinding.id = keyBinding.id ?? keyBinding.name;
                let existing = this.getBind(keyBinding.id);
                if (existing) {
        ...
        constructor(config){
            if (this.addKeys = this.addKey.bind(this), this.keysPressed = [], this.binds = [], this.tickers = [], this.config = KeyManager.configManager.parse(config), this.config.autoAddInterval) {
            ...
            this.tickers.push((dt)=>{
                for (let bind of this.binds)(void 0 !== bind.onDownContinuous || void 0 !== bind.fn) && this.isPressing(bind.id) && (bind.onDownContinuous?.(dt), bind.fn?.(dt));
            }), document.addEventListener("keydown", (e)=>{
                this.logKey(e, !0), this.onAll("down", e.key);
            }), document.addEventListener("keyup", (e)=>{
                this.logKey(e, !1), this.onAll("up", e.key);
            }), document.addEventListener("keypress", (e)=>{
                this.onAll("press", e.key);
            });
        }
    }
    KeyManager.configManager = new ConfigManager({
        autoAddInterval: !0,
        fps: 30
    });

    var EventTypes, EventTypes1 = ((EventTypes = EventTypes1 || {}).interval = "interval", EventTypes.timeout = "timeout", EventTypes);
    class EventManager {
        on(event, callback) {
            this.callbackEvents[event] || (this.callbackEvents[event] = []), this.callbackEvents[event].push({
                type: event,
                callback
            });
        }
        dispatch(event) {
            if (this.callbackEvents[event]) for (let callback of this.callbackEvents[event])callback.callback();
        }
        tickerFunction() {
            let currentTime = Date.now();
            for (let event of Object.values(this.events))switch(event.type){
                case "interval":
                    if (currentTime - event.intervalLast >= event.delay) {
                        let dt = currentTime - event.intervalLast;
                        event.callback(dt), event.intervalLast = currentTime;
                    }
                    break;
                case "timeout":
                    if (currentTime - event.timeCreated >= event.delay) {
                        let dt = currentTime - event.timeCreated;
                        event.callback(dt), delete this.events[event.name];
                    }
            }
        }
        changeFps(fps) {
        ...
        setEvent(nameOrEvent, type, delay, callbackFn) {
            let eventToAdd = {
                type: "timeout",
                delay: 0,
                ..."string" != typeof nameOrEvent ? nameOrEvent : {
                    name: nameOrEvent,
                    type: type,
                    delay: delay,
                    callback: callbackFn
                ...
                intervalLast: "interval" === type ? Date.now() : 0
            };
        ...
        constructor(config, events){
            if (this.addEvent = this.setEvent.bind(this), this.config = EventManager.configManager.parse(config), this.events = {}, this.callbackEvents = {}, events) for (let event of events)this.callbackEvents[event] = [];
            if (this.config.autoAddInterval) {
                let fps = this.config.fps ?? 30;
                this.tickerInterval = setInterval(()=>{
                    this.tickerFunction();
                }, 1000 / fps);
            }
    ...
    EventManager.configManager = new ConfigManager({
        autoAddInterval: !0,
        fps: 30
    });
    		...
    		var LZString = function() {
    		    ...
    		    var LZString = {
    		        ...
    		        decompressFromUint8Array: function(compressed) {
    		            ...
    		            for(var buf = Array(compressed.length / 2), i = 0, TotalLen = buf.length; i < TotalLen; i++)buf[i] = 256 * compressed[2 * i] + compressed[2 * i + 1];
    		            var result = [];
    		        ...
    		        _compress: function(uncompressed, bitsPerChar, getCharFromInt) {
    		            ...
    		            for(ii = 0; ii < uncompressed.length; ii += 1)if (context_c = uncompressed.charAt(ii), Object.prototype.hasOwnProperty.call(context_dictionary, context_c) || (context_dictionary[context_c] = context_dictSize++, context_dictionaryToCreate[context_c] = !0), context_wc = context_w + context_c, Object.prototype.hasOwnProperty.call(context_dictionary, context_wc)) context_w = context_wc;
    		                ...
    		                if (Object.prototype.hasOwnProperty.call(context_dictionaryToCreate, context_w)) {
    		                    if (256 > context_w.charCodeAt(0)) {
    		                        ...
    		                        for(i = 0, value = context_w.charCodeAt(0); i < 8; i++)context_data_val = context_data_val << 1 | 1 & value, context_data_position == bitsPerChar - 1 ? (context_data_position = 0, context_data.push(getCharFromInt(context_data_val)), context_data_val = 0) : context_data_position++, value >>= 1;
    		                    } else {
    		                        for(i = 0, value = 1; i < context_numBits; i++)context_data_val = context_data_val << 1 | value, context_data_position == bitsPerChar - 1 ? (context_data_position = 0, context_data.push(getCharFromInt(context_data_val)), context_data_val = 0) : context_data_position++, value = 0;
    		                        for(i = 0, value = context_w.charCodeAt(0); i < 16; i++)context_data_val = context_data_val << 1 | 1 & value, context_data_position == bitsPerChar - 1 ? (context_data_position = 0, context_data.push(getCharFromInt(context_data_val)), context_data_val = 0) : context_data_position++, value >>= 1;
    		                    }
    		                ...
    		                } else for(i = 0, value = context_dictionary[context_w]; i < context_numBits; i++)context_data_val = context_data_val << 1 | 1 & value, context_data_position == bitsPerChar - 1 ? (context_data_position = 0, context_data.push(getCharFromInt(context_data_val)), context_data_val = 0) : context_data_position++, value >>= 1;
    		            ...
    		            for(i = 0, value = 2; i < context_numBits; i++)context_data_val = context_data_val << 1 | 1 & value, context_data_position == bitsPerChar - 1 ? (context_data_position = 0, context_data.push(getCharFromInt(context_data_val)), context_data_val = 0) : context_data_position++, value >>= 1;
    		            for(;;){
    		        ...
    		        _decompress: function(length, resetValue, getNextValue) {
    		            ...
    		            for(bits = 0, maxpower = 4, power = 1; power != maxpower;)resb = data.val & data.position, data.position >>= 1, 0 == data.position && (data.position = resetValue, data.val = getNextValue(data.index++)), bits |= (resb > 0 ? 1 : 0) * power, power <<= 1;
    		            switch(bits){
    		                case 0:
    		                    for(bits = 0, maxpower = 256, power = 1; power != maxpower;)resb = data.val & data.position, data.position >>= 1, 0 == data.position && (data.position = resetValue, data.val = getNextValue(data.index++)), bits |= (resb > 0 ? 1 : 0) * power, power <<= 1;
    		                    c = f(bits);
    		                    break;
    		                case 1:
    		                    for(bits = 0, maxpower = 65536, power = 1; power != maxpower;)resb = data.val & data.position, data.position >>= 1, 0 == data.position && (data.position = resetValue, data.val = getNextValue(data.index++)), bits |= (resb > 0 ? 1 : 0) * power, power <<= 1;
    		                    c = f(bits);
    		                    break;
    		                case 2:
    		                    return "";
    		            ...
    		            for(dictionary[3] = c, w = c, result.push(c);;){
    		                ...
    		                result.push(entry), dictionary[dictSize++] = w + entry.charAt(0), enlargeIn--, w = entry, 0 == enlargeIn && (enlargeIn = Math.pow(2, numBits), numBits++);
    		            }
    		        }
    		    };
    		    return LZString;
    		}();
    		null != module ? module.exports = LZString : 'undefined' != typeof angular && null != angular && angular.module('LZString', []).factory('LZString', function() {
    		    return LZString;
    		}); 
    	} (lzString));
    ...
    function requireCrypt () {
    	...
    	hasRequiredCrypt = 1;
    	var base64map, crypt$1;
    	base64map = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/', crypt$1 = {
    	    rotl: function(n, b) {
    	        return n << b | n >>> 32 - b;
    	    },
    	    rotr: function(n, b) {
    	    ...
    	    endian: function(n) {
    	        if (n.constructor == Number) return 0x00FF00FF & crypt$1.rotl(n, 8) | 0xFF00FF00 & crypt$1.rotl(n, 24);
    	    ...
    	    wordsToBytes: function(words) {
    	        for(var bytes = [], b = 0; b < 32 * words.length; b += 8)bytes.push(words[b >>> 5] >>> 24 - b % 32 & 0xFF);
    	        return bytes;
    	    },
    	    bytesToHex: function(bytes) {
    	        for(var hex = [], i = 0; i < bytes.length; i++)hex.push((bytes[i] >>> 4).toString(16)), hex.push((0xF & bytes[i]).toString(16));
    	        return hex.join('');
    	    },
    	    ...
    	    bytesToBase64: function(bytes) {
    	        for(var base64 = [], i = 0; i < bytes.length; i += 3)for(var triplet = bytes[i] << 16 | bytes[i + 1] << 8 | bytes[i + 2], j = 0; j < 4; j++)8 * i + 6 * j <= 8 * bytes.length ? base64.push(base64map.charAt(triplet >>> 6 * (3 - j) & 0x3F)) : base64.push('=');
    	        return base64.join('');
    	    },
    ...
    function requireCharenc () {
    	...
    	var charenc = {
    	    ...
    	    bin: {
    	        stringToBytes: function(str) {
    	            for(var bytes = [], i = 0; i < str.length; i++)bytes.push(0xFF & str.charCodeAt(i));
    	            return bytes;
    	        },
    	        bytesToString: function(bytes) {
    ...
    function requireIsBuffer () {
    	...
    	isBuffer_1 = function(obj) {
    	    return null != obj && (isBuffer(obj) || 'function' == typeof obj.readFloatLE && 'function' == typeof obj.slice && isBuffer(obj.slice(0, 0)) || !!obj._isBuffer);
    	};
    	return isBuffer_1;
    }
    ...
    function requireMd5 () {
    	...
    	crypt = requireCrypt(), utf8 = requireCharenc().utf8, isBuffer = requireIsBuffer(), bin = requireCharenc().bin, (md5 = function(message, options) {
    	    message.constructor == String ? message = options && 'binary' === options.encoding ? bin.stringToBytes(message) : utf8.stringToBytes(message) : isBuffer(message) ? message = Array.prototype.slice.call(message, 0) : Array.isArray(message) || message.constructor === Uint8Array || (message = message.toString());
    	    for(var m = crypt.bytesToWords(message), l = 8 * message.length, a = 1732584193, b = -271733879, c = -1732584194, d = 271733878, i = 0; i < m.length; i++)m[i] = (m[i] << 8 | m[i] >>> 24) & 0x00FF00FF | (m[i] << 24 | m[i] >>> 8) & 0xFF00FF00;
    	    m[l >>> 5] |= 0x80 << l % 32, m[(l + 64 >>> 9 << 4) + 14] = l;
    	    for(var FF = md5._ff, GG = md5._gg, HH = md5._hh, II = md5._ii, i = 0; i < m.length; i += 16){
    	    ...
    	    var n = a + (c ^ (b | ~d)) + (x >>> 0) + t;
    	    return (n << s | n >>> 32 - s) + b;
    	}, md5._blocksize = 16, md5._digestsize = 16, md5$1.exports = function(message, options) {
    	    if (null == message) throw Error('Illegal argument ' + message);
    ...
    function isPlainObject(obj) {
        return "object" == typeof obj && obj?.constructor === Object;
    }
    let objectHasOwnProperty = (obj, key)=>Object.prototype.hasOwnProperty.call(obj, key), upgradeDataProperties = Object.getOwnPropertyNames(new UpgradeData({
        id: "",
        level: Decimal.dZero
    })), itemDataProperties = Object.getOwnPropertyNames(new ItemData({
        id: "",
    ...
    class DataManager {
        ...
        setData(key, value) {
            void 0 === this.data[key] && this.normalData && console.warn("eMath.js: After initializing data, you should not add new properties to data."), this.data[key] = value;
            ...
            return {
                ...
                setValue (valueToSet1) {
                    thisData()[key] = valueToSet1;
                }
            };
        }
        getData(key) {
        ...
        setStatic(key, value) {
            return console.warn("eMath.js: setStatic: Static data is basically useless and should not be used. Use variables in local scope instead."), void 0 === this.static[key] && this.normalData && console.warn("eMath.js: After initializing data, you should not add new properties to staticData."), this.static[key] = value, this.static[key];
        }
        getStatic(key) {
            return console.warn("eMath.js: Static data is basically useless and should not be used. Use variables in local scope instead."), this.static[key];
        }
        init() {
            this.normalData = this.data, this.normalDataPlain = classTransformer.instanceToPlain(this.data);
        }
        compileDataRaw(data = this.data) {
            this.gameRef.eventManager.dispatch("beforeCompileData");
            let gameDataString = classTransformer.instanceToPlain(data);
            return [
                {
                    hash: md5(`${this.gameRef.config.name.id}/${JSON.stringify(gameDataString)}`),
                    game: {
        ...
        compileData(data = this.data) {
            return lzStringExports.compressToBase64(JSON.stringify(this.compileDataRaw(data)));
        }
        decompileData(data) {
            if (!data) {
                if (!this.localStorage) return console.warn("eMath.js: Local storage is not supported. Methods that rely on local storage will not work: decompileData() requires the data to be passed as an argument."), null;
                data = this.localStorage.getItem(`${this.gameRef.config.name.id}-data`);
            }
            if (!data) return null;
            try {
                return JSON.parse(lzStringExports.decompressFromBase64(data));
            } catch (error) {
                if (error instanceof SyntaxError) console.error(`Failed to decompile data (corrupted) "${data}":`, error);
                else throw error;
                return null;
            }
        }
        validateData(data) {
            let [saveMetadata, gameDataToValidate] = data;
            return "string" == typeof saveMetadata ? md5(`${this.gameRef.config.name.id}/${JSON.stringify(gameDataToValidate)}`) === saveMetadata : saveMetadata.hash === md5(`${this.gameRef.config.name.id}/${JSON.stringify(gameDataToValidate)}`);
        }
        resetData(reload = !1) {
            if (!this.normalData) throw Error("dataManager.resetData(): You must call init() before writing to data.");
            this.data = this.normalData, this.saveData(), reload && window.location.reload();
        }
        saveData(dataToSave = this.compileData()) {
            if (this.gameRef.eventManager.dispatch("beforeSaveData"), !dataToSave) throw Error("dataManager.saveData(): Data to save is empty.");
            if (!this.localStorage) throw Error("dataManager.saveData(): Local storage is not supported. You can use compileData() instead to implement a custom save system.");
            this.localStorage.setItem(`${this.gameRef.config.name.id}-data`, dataToSave), this.gameRef.eventManager.dispatch("saveData");
        }
        exportData() {
            if ("undefined" == typeof document) {
                console.warn("eMath.js: exportData(): Document is not defined. You can use compileData() instead to implement a custom save system.");
                return;
            }
            let content = this.compileData();
            if (null != prompt("Download save data?:", content)) {
                let blob = new Blob([
                    content
                ], {
                    type: "text/plain"
                }), downloadLink = document.createElement("a");
                downloadLink.href = URL.createObjectURL(blob), downloadLink.download = `${this.gameRef.config.name.id}-data.txt`, downloadLink.textContent = `Download ${this.gameRef.config.name.id}-data.txt file`, document.body.appendChild(downloadLink), downloadLink.click(), document.body.removeChild(downloadLink);
            }
        }
        parseData(dataToParse = this.decompileData(), mergeData = !0) {
            if ((!this.normalData || !this.normalDataPlain) && mergeData) throw Error("dataManager.parseData(): You must call init() before writing to data.");
            if (!dataToParse) return null;
            let [, loadedData] = dataToParse, loadedDataMerged = mergeData ? function deepMerge(sourcePlain, source, target) {
                if (!sourcePlain || !source || !target) return console.warn("eMath.js: dataManager.deepMerge(): Missing arguments:", sourcePlain, source, target), target ?? {};
                for(let key in sourcePlain)if (objectHasOwnProperty(sourcePlain, key) && !objectHasOwnProperty(target, key) && (target[key] = sourcePlain[key]), source[key] instanceof Currency) {
                    let sourceCurrency = sourcePlain[key], targetCurrency = target[key];
                    if (Array.isArray(targetCurrency.upgrades)) {
                        let upgrades = targetCurrency.upgrades;
                        for (let upgrade of (targetCurrency.upgrades = {}, upgrades))targetCurrency.upgrades[upgrade.id] = upgrade;
                    }
                    targetCurrency.upgrades = {
                        ...sourceCurrency.upgrades,
                        ...targetCurrency.upgrades
                    }, target[key] = targetCurrency, targetCurrency.items = {
                        ...sourceCurrency.items,
                        ...targetCurrency.items
                    };
                } else isPlainObject(sourcePlain[key]) && isPlainObject(target[key]) && (target[key] = deepMerge(sourcePlain[key], source[key], target[key]));
                return target;
            }(this.normalDataPlain, this.normalData, loadedData) : loadedData;
            return function plainToInstanceRecursive(normal, plain) {
                if (!normal || !plain) throw Error("dataManager.plainToInstanceRecursive(): Missing arguments.");
                for(let key in normal){
                    ...
                    let normalDataClass = normal[key].constructor;
                    if (normalDataClass === Object) {
                        plain[key] = plainToInstanceRecursive(normal[key], plain[key]);
                        continue;
                    }
                    plain[key] = function(templateClassToConvert, plain) {
                        let out = classTransformer.plainToInstance(templateClassToConvert, plain);
                        if (out instanceof Currency) {
                            for(let upgradeName in out.upgrades){
                                let upgrade = out.upgrades[upgradeName];
                                ...
                                out.upgrades[upgradeName] = classTransformer.plainToInstance(UpgradeData, upgrade);
                            }
                            for(let itemName in out.items){
                                let item = out.items[itemName];
                                if (!item || !itemDataProperties.every((prop)=>Object.getOwnPropertyNames(item).includes(prop))) {
                                    delete out.items[itemName];
                                    continue;
                                }
                                out.items[itemName] = classTransformer.plainToInstance(ItemData, item);
                            }
                        }
                        if (!out) throw Error(`Failed to convert ${templateClassToConvert.name} to class instance.`);
                        return out;
                    }(normalDataClass, plain[key]);
                }
                return plain;
            }(this.normalData, loadedDataMerged);
        }
        loadData(dataToLoad = this.decompileData()) {
            if (!(dataToLoad = "string" == typeof dataToLoad ? this.decompileData(dataToLoad) : dataToLoad)) return null;
            let isDataValid = this.validateData([
                dataToLoad[0],
                classTransformer.instanceToPlain(dataToLoad[1])
            ]), parsedData = this.parseData(dataToLoad);
            if (!parsedData) return null;
            for (let obj of (this.data = parsedData, this.eventsOnLoad))obj();
            return this.gameRef.eventManager.dispatch("loadData"), isDataValid;
        }
        constructor(gameRef, localStorage){
            this.data = {}, this.static = {}, this.eventsOnLoad = [], this.gameRef = "function" == typeof gameRef ? gameRef() : gameRef, this.localStorage = localStorage ?? ("undefined" == typeof window ? (console.warn("eMath.js: Local storage is not supported. Methods that rely on local storage will not work. You can use compileData() and decompileData() instead to implement a custom save system."), null) : window.localStorage);
        }
    }

    class GameCurrency extends CurrencyStatic {
        get data() {
            return this.pointer;
        }
        get static() {
        ...
        constructor(currencyStaticParams, gamePointer, name){
            if ("function" == typeof currencyStaticParams) throw Error("GameCurrency constructor does not accept a function as the first parameter. Use the <Game>.addCurrency method instead.");
            super(...currencyStaticParams), this.game = gamePointer, this.name = name, this.game.dataManager.addEventOnLoad(()=>{
                this.static.onLoadData();
            });
        }
    }

    class GameAttribute extends AttributeStatic {
        get data() {
            return this.pointer;
        }
        get static() {
        ...
        constructor(attributeStaticParams, gamePointer){
            if ("function" == typeof attributeStaticParams) throw Error("GameAttribute constructor does not accept a function as the first parameter. Use the <Game>.addAttribute method instead.");
            super(...attributeStaticParams), this.game = gamePointer;
        }
    }
    ...
    class GameReset {
        ...
        reset(force = !1, forceExtenders = !0, cached = new Set()) {
            ...
            this.extender.forEach((extender)=>{
                cached.has(extender.id) || (cached.add(extender.id), extender.reset(forceExtenders || force, forceExtenders, cached));
            }), resetThis();
        }
        constructor(currenciesToReset, extender, onReset, condition){
            this.currenciesToReset = Array.isArray(currenciesToReset) ? currenciesToReset : [
                currenciesToReset
            ], extender = extender ?? [], this.extender = Array.isArray(extender) ? extender : [
                extender
            ], this.onReset = onReset, this.condition = condition, this.id = Symbol();
        }
    }

    let gameDefaultConfig = {
        mode: "production",
        name: {
            title: "",
            id: "",
    ...
    class Game {
        ...
        changeFps(fps) {
            this.keyManager.changeFps(fps), this.eventManager.changeFps(fps);
        }
        addCurrency(name, upgrades = [], items = []) {
            return this.dataManager.setData(name, {
        ...
        addAttribute(name, useBoost = !0, initial = 0) {
            return this.dataManager.setData(name, new Attribute(initial)), new GameAttribute([
        ...
        addReset(...args) {
            return console.warn("eMath.js: Game.addReset is deprecated. Use the GameReset class instead."), new GameReset(...args);
        }
        addResetFromObject(object) {
            return console.warn("eMath.js: Game.addResetFromObject is deprecated. Use the GameReset.fromObject static method instead."), GameReset.fromObject(object);
        }
        constructor(config){
            this.config = Game.configManager.parse(config), this.dataManager = new DataManager(this, this.config.localStorage), this.keyManager = new KeyManager({
    ...
    exports.EventTypes = EventTypes1;
    exports.Game = Game;
    exports.GameAttribute = GameAttribute;
    exports.GameCurrency = GameCurrency;
    exports.GameReset = GameReset;
    exports.KeyManager = KeyManager;
    exports.gameDefaultConfig = gameDefaultConfig;
    exports.keys = keys;
