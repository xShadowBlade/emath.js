...
coins.static.addUpgrade({
    ...
    //     const costFn = (a: Decimal, b: Decimal) => new Decimal(-5).mul(a.sub(b).sub(1)).mul(a.add(b));
    ...
    //     let maxAffordable = Decimal.floor(Decimal.sqrt(discriminant).sub(1).div(2)).sub(level);
    //     maxAffordable = target.gt(0) ? Decimal.min(maxAffordable, (target.sub(1))) : maxAffordable;
    ...
    //     return [cost, maxAffordable.add(1)] as [Decimal, Decimal];
    // },
...
function updateDisplay () {
    // coinsDisplay!.innerHTML = `Coins: ${coins.value.format()} (${Decimal.formats.formatMult(coins.static.boost.calculate())})`; // Updates the display and shows the multiplier. Ex. "Coins: 2.00 (x1.0)"
    coinsDisplay!.innerHTML = `Coins: ${coins.value.toString()} (${coins.static.boost.calculate().toString()})x`; // Updates the display and shows the multiplier. Ex. "Coins: 2.00 (x1.0)"
}
updateDisplay();
...
function updateDisplayUpgrade () {
    buyUpgradesButton!.innerHTML = `Buy ${coins.static.calculateUpgrade("upg1Coins", 25)[0].toString()} Upgrades for ${coins.static.calculateUpgrade("upg1Coins", 25)[1].toString()} Coins (b)`;
}
updateDisplayUpgrade();
